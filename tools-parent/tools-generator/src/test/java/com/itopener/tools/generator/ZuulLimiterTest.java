...
public class ZuulLimiterTest {
	...
	public void init(){
		try {
			...
			config.setBasePackageModel("com.itopener.tools.zuul.ratelimiter.admin.model");
			config.setBasePackageMapper("com.itopener.tools.zuul.ratelimiter.admin.mapper");
			config.setBasePackageDao("com.itopener.tools.zuul.ratelimiter.admin.dao");
			config.setBasePackageCondition("com.itopener.tools.zuul.ratelimiter.admin.conditions");
//			config.setImplementsInterface("BaseCondition");
			config.setBaseDao("com.itopener.framework.base.BaseDao");
			config.setBasePath("D:\\workspace-neon\\itopener\\itopener-parent\\tools-parent\\tools-zuul-ratelimiter-admin-parent\\tools-zuul-ratelimiter-admin-model\\src\\main\\java\\");
			config.setDriver("com.mysql.jdbc.Driver");
			config.setUrl("jdbc:mysql://127.0.0.1:3306/itopener_demo?useUnicode=true&characterEncoding=utf8&autoReconnect=true");
			config.setPassword("root");
			config.setUser("root");
			config.setTables("zuul_id_limiter,zuul_path_limiter");
...
//		config.getWhereColumns().get("sys_dictionary").add("id");
			...
			config.getBaseWhereColumns().add("zuul_id");
			config.getBaseWhereColumns().add("path");
